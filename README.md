...
- One-click guest entry with an isolated browser workspace for every visitor
...
| `GUEST_SESSION_SECRET` | Signs isolated guest-workspace tokens | Set a long random value in Railway |
...
The repository is configured for a split deployment: Vercel serves the React
frontend, while Railway runs the FastAPI Docker service and PostgreSQL.

1. In Railway, deploy the GitHub repository and add PostgreSQL to the project.
2. Set `DATABASE_URL` from PostgreSQL, plus `ENVIRONMENT=production`,
...
The root `Dockerfile` and `railway.json` run database migrations before each
Railway start. Guest identity is transported as a signed token so isolation
continues to work across the separate Vercel and Railway domains.

## Current boundaries

- Guest workspaces are isolated with a signed browser token. They are
