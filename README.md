...
The test suite covers workflow composition, standalone chats, deletion preservation, title and message-content search, same-conversation foreign keys, branch ancestry sharing, summary pruning, reference prompts, all three prompt resolutions, nested branches, stale cursors, and provider failure rollback.
...
- Single-user workspace; authentication is intentionally out of scope.
...
- The included deployment is intentionally public and single-user: every visitor
  shares the same chats, database, and Gemini quota.
